...
### Sonido Golpear la raqueta:
- link: https://freesound.org/people/JustInvoke/sounds/446100/
- 

### Sonido al aplicar un gol en el juego:
...
## Narracion en el juego

- cuando se marque un gol que se narre el marcador

Prompt: "cuando se marque un gol narrar el marcador"
...
```javascript
    ...
    function narrateScore() {
        ...
        if (voices.length === 0) {
            console.log('No hay voces disponibles en este navegador.');
            return;
        }

        let msg = new SpeechSynthesisUtterance();
        msg.text = `El jugador tiene ${playerScore} puntos. El computador tiene ${computerScore} puntos.`;
        msg.volume = 1;
        msg.rate = 1;
        msg.pitch = 1;
        msg.voice = voices[0]; // Usar la primera voz disponible
    ...
    class Ball {
        ...
        checkScore() {
            if (this.x < PADDING) {
                computerScore++;
                gameOverSound.play();  // Reproducir sonido de game over
                narrateScore();        // Narrar el puntaje
                this.reset();
            } else if (this.x > width - PADDING) {
                playerScore++;
                gameOverSound.play();  // Reproducir sonido de game over
...
- Se hicieron multiples pruebas pero no funcionó, se agregó una api pero da error.
- En la indicada en el curso dice que el navegador no tiene una voz 
- se verifica con el original del curso y tampoco suena la narracion.

![agregando narracion](/imagenes/clase04/agregando_narracion.png)